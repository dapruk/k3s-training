...
Add Deployment and Pod-template annotation `training.hairnerds.id/deployed-by: akmal`. Pod template also needs `training.hairnerds.id/git-sha: IMAGE_GIT_SHA`. Do not use `imagePullPolicy: Never`. Do not create Namespace, Ingress, PV, or PVC. Backend image: `<registry>/k3s-training/fullstack-be:akmal-<git-sha>`.
